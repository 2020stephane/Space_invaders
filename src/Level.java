...
public class Level {
    ...
    public Level(int level) {
        ...
        MyPanel.ListEnemy = new ArrayList<>();
    ...
    private void ImageLoader() {
        try {
            ...
            MyPanel.bufferlost = ImageIO.read(Objects.requireNonNull
                    (MainClass.class.getResource("image/moonperdu.png")));
            ...
            for (int col = 0; col < 6; col ++) {
                Game.joueur1.setImage_explosion(col, MyPanel.explosionJ.getSubimage(
                           col * 65, 0, 65, 70));
                }
    ...
    private void DrawEnnemy(int level) {
            ...
            for (int x = 1; x <= 20; x++) {
                MyPanel.ListEnemy.add(new Enemy(x * 75, 10, TypeSprite.ENNEMY1));
            }
            for (int x = 2; x <= 19; x++) {
                MyPanel.ListEnemy.add(new Enemy(x * 75, 85, TypeSprite.ENNEMY2));

            }
            for (int x = 3; x <= 18; x++) {
                MyPanel.ListEnemy.add(new Enemy(x * 75, 160, TypeSprite.ENNEMY1));

            }
            for (int x = 4; x <= 17; x++) {
                MyPanel.ListEnemy.add(new Enemy(x * 75, 235, TypeSprite.ENNEMY1));

            }
            for (int x = 5; x <= 16; x++) {
                MyPanel.ListEnemy.add(new Enemy(x * 75, 310, TypeSprite.ENNEMY2));

            }
            for (int x = 6; x <= 15; x++) {
                MyPanel.ListEnemy.add(new Enemy(x * 75, 385, TypeSprite.ENNEMY1));

            }
         if (level == 2) {
             for (int x = 1; x <= 20; x++) {
                 MyPanel.ListEnemy.add(new Enemy(x * 75, 450, TypeSprite.ENNEMY1));

             }
             for (int x = 2; x <= 19; x++) {
                 MyPanel.ListEnemy.add(new Enemy(x * 75, 525, TypeSprite.ENNEMY2));

             }
             for (int x = 3; x <= 18; x++) {
                 MyPanel.ListEnemy.add(new Enemy(x * 75, 600, TypeSprite.ENNEMY1));
